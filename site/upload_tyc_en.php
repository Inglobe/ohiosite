<?php
	...
	if( is_uploaded_file($_FILES['tyc']['tmp_name']) ){
		$data = array('success' => true);
	}
	if($_FILES['tyc']['type']!='application/pdf'){
		header('Location: index.php?upstatus=err#portfolio');
	}else {
		$origen = $_FILES['tyc']['tmp_name'];
		$_FILES['tyc']['name']='tcpp.pdf';
	    $destino = '../terms and conditions/en/'.$_FILES['tyc']['name'];
